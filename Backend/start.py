...
#
# Strategic Plan API (backend)
#
# This module implements a FastAPI application that provides endpoints for
# managing strategic plans, objectives, goals, indicators, resources and
# evidences. Key responsibilities include:
# - Authentication and authorization (OAuth2 password flow + role checks)
# - Database models and SQLAlchemy ORM mappings for plans, objectives, users
# - CRUD endpoints for plans, objectives, uploads, and evidence handling
# - Utilities for RUT normalization, file validation, cloud uploads (Cloudinary)
# - Helpers to obtain the current user, require roles, and to open/close DB sessions
#
# Implementation notes:
# - The app uses SQLAlchemy (declarative models) and a session factory.
# - Security uses JWT tokens and a simple `require_role` dependency for role-based access.
# - Uploads are accepted to `./uploads` and may be proxied to Cloudinary.
# - The file contains many helper utilities and should be read top-down for
#   models, endpoints and business logic.
#
...
async def upload_plan_evidence(plan_id: int, file: UploadFile = File(...), description: str = Form(""), db: Session = Depends(get_db), user: "UserModel" = Depends(require_role("editor"))):
