/*
  ...
  Purpose: Initialize and handle the Plan form used to create/update
  strategic action plans (acciones/planes) from the frontend.

  Main responsibilities:
  - Populate and lock the school field and load available dimensions from API.
  - Provide helpers to collect and calculate resource fields (auto-total logic).
  - Validate required fields and submit plan data to the backend (`/plans`).
  - Optionally create related resources via `/plans/{id}/resources` when provided.

  Notes:
  - Exposes `initPlanForm` to `window` so it can be called from the form page.
  - Uses a few helper functions for numeric parsing and payload collection.
*/
