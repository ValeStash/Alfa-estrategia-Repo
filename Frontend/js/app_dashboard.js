...
/** Pequeños helpers para las mini barras en tabla de Indicadores */
function formatPct1(n){
  const v = Math.max(0, Math.min(100, Number(n) || 0));
  return String(Math.round(v * 10) / 10);
}
function setMiniBarValue(selector, pct){
  const el = document.querySelector(selector);
  if (!el) return;
  const clamp = Math.max(0, Math.min(100, Number(pct) || 0));
  el.style.setProperty('--value', String(clamp / 100));
  el.setAttribute('data-pct', formatPct1(clamp));
}
...
async function showIndicatorsForGoal(goalId) {
  ...
  function paintTable(indics) {
    ...
    $tb.innerHTML = indics.map((x, idx) => {
      ...
      // Unidad: mostrar EN VIVO el valor de "Cantidad obtenida" (centrado)
      const unitCell = (kind === 'percent' || kind === 'count')
        ? `<div style="display:flex;justify-content:center;align-items:center;min-height:20px;">
             <span id="unit-obt-${x.id}" class="unit-obt">—</span>
           </div>`
        : `${esc(x.unit || '')}`;
      // Celda de Progreso (barra) para indicadores porcentuales
      const progCell = (kind === 'percent')
  ? `<span class="mini__bar" id="row-bar-${x.id}" style="--width:200px; --height:18px" data-pct="0"></span>`
        : '—';
      return `
    ...
    // Pinta las mini bars a la derecha (si existe el panel)
    const pane = document.getElementById('indBars');
    if (pane) {
      pane.innerHTML = indics.map(x => `
        <div class="ind-bar-row">
          <span class="mini__bar" id="mini-bar-${x.id}" style="--width:100%; --height:18px" data-pct="0"></span>
        </div>
      `).join('');
    }
  ...
  function computeAndRenderProgress(id, kind) {
    ...
    if (kind === 'percent' || kind === 'count') {
      ...
      const $unitObt = document.getElementById(`unit-obt-${id}`);

      if (!Number.isFinite(total) || total <= 0) {
        $out.innerHTML = `<strong>Resultado:</strong> —`;
        setMiniBarValue(`#row-bar-${id}`, 0);
        if ($unitObt) $unitObt.textContent = '—';
        return;
      }
      if (!Number.isFinite(obt)   || obt < 0) {
        $out.innerHTML = `<strong>Resultado:</strong> —`;
        setMiniBarValue(`#row-bar-${id}`, 0);
        if ($unitObt) $unitObt.textContent = '—';
        return;
      ...
      if (kind === 'percent') {
        ...
        setMiniBarValue(`#row-bar-${id}`, pct); // sincroniza Progreso con Resultado
        if ($unitObt) $unitObt.textContent = String(obt);
